...
MMP_RESULT CMmpDemuxer_FfmpegEx1::Open()
{
    ...
    if(mmpResult == MMP_SUCCESS) {
        ...
        m_pAVIOContext = avio_alloc_context(m_IoBuffer, io_buffer_size, 0, m_Url, CMmpDemuxer_FfmpegEx1::IORead_C_Stub, NULL, CMmpDemuxer_FfmpegEx1::IOSeek_C_Stub);
...
MMP_U32 CMmpDemuxer_FfmpegEx1::GetAudioFormat() {
...
MMP_U32 CMmpDemuxer_FfmpegEx1::GetVideoFormat() {
...
#if 0
MMP_RESULT CMmpDemuxer_FfmpegEx1::GetNextVideoData(MMP_U8* buffer, MMP_U32 buf_max_size, MMP_U32* buf_size, MMP_S64* packt_pts) {
...
MMP_RESULT CMmpDemuxer_FfmpegEx1::GetNextMediaData(MMP_U32 mediatype, MMP_U8* buffer, MMP_U32 buf_max_size, MMP_U32* buf_size, MMP_S64* packt_pts) {
        ...
        if(pkt.stream_index == m_nStreamIndex[mediatype]) {
            ...
            if(packt_pts) {
               ...
               // }
                ...
                MMPDEBUGMSG(0, (TEXT("[CMmpDemuxer_FfmpegEx1::GetNextMediaData] MediaType=(%s)  pts=%d packsz=%d  pack(pts=%d dts=%d start=%d num=%d den=%d ) \n\r"),
                                      szMediaType,
...
int CMmpDemuxer_FfmpegEx1::IORead_C_Stub(void *opaque, uint8_t *buf, int buf_size) {

    URLContext *p_url = (URLContext*)opaque;
    CMmpDemuxer_FfmpegEx1 *pObj = (CMmpDemuxer_FfmpegEx1 *)p_url->priv_data;
    return pObj->IORead_C(opaque, buf, buf_size);
}

int64_t CMmpDemuxer_FfmpegEx1::IOSeek_C_Stub(void *opaque, int64_t offset, int whence) {
...
int CMmpDemuxer_FfmpegEx1::IORead_C(void *opaque, uint8_t *buf, int buf_size) {
    ...
    if(buffer_remain_byte == 0) {
        ...
        //MMPDEBUGMSG(1, (TEXT("[CMmpDemuxer_FfmpegEx1::IORead_C]  Burst Read 1 (%d/%d  %d) "),  m_nFileBufferSize, MMP_DEMUXER_FILE_BUFFER_MAX_SIZE, buf_size));
    }

    buffer_remain_byte = m_nFileBufferSize - m_nFileBufferIndex;

    if(m_nFileBufferSize < 0) {
        i_ret = -1;
        m_nFileBufferSize = 0;
        m_nFileBufferIndex = 0;
    }   
    else if(buffer_remain_byte == 0) {
        i_ret = 0;
    }
        ...
        while(buf_size1 > 0) {
            ...
            MMPDEBUGMSG(1, (TEXT("[CMmpDemuxer_FfmpegEx1::IORead_C]  Burst Read 2 ")));
            m_nFileBufferSize = fread(m_FileBuffer, 1, MMP_DEMUXER_FILE_BUFFER_MAX_SIZE, m_fp);
            m_nFileBufferIndex = 0;
...
int64_t CMmpDemuxer_FfmpegEx1::IOSeek_C(void *opaque, int64_t offset, int whence) {
    ...
    while(1) {
        ...
        if(empty_stream_index != -1) {
            ...
            if(av_read_frame(m_pAvformatCtx, &pkt ) )
            {
                ...
                MMPDEBUGMSG(0, (TEXT("[CMmpDemuxer_FfmpegEx1::queue_buffering] MediaType=(%d %s)  pts=%d packsz=%d  pack(pts=%d dts=%d start=%d num=%d den=%d ) \n\r"),
