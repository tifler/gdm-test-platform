...
#if (MMP_OS == MMP_OS_WIN32)
extern int vpu_open(char* drvname, int flag);
extern int vpu_close(int fd);
extern int vpu_ioctl(int fd, int cmd, void* parm);
extern void *vpu_mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset);

#else
#define vpu_open  open
#define vpu_close close
#define vpu_ioctl ioctl
#define vpu_mmap mmap
#endif
...
int vdi_init(unsigned long coreIdx)
{
    ...
    vdi->vpu_fd = vpu_open(VPU_DEVICE_NAME, O_RDWR); // if this API supports VPU parallel processing using multi VPU. the driver should be made to open multiple times.
	if (vdi->vpu_fd < 0) {
	...
	if(!vdi->pvip->instance_pool_inited)
	{
		...
		pthread_mutex_init((MUTEX_HANDLE *)vdi->vpu_omx_mutex, &mutexattr);
	...
	vdi->vdb_register.virt_addr = (unsigned long)vpu_mmap(NULL, vdi->vdb_register.size, PROT_READ | PROT_WRITE, MAP_SHARED, vdi->vpu_fd, 0);
...
int allocate_common_memory(unsigned long coreIdx)
{
	...
	if (vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_GET_COMMON_MEMORY, &vdb) < 0)
	{
		VLOG(ERR, "[VDI] fail to vdi_allocate_dma_memory size=%d\n", vdb.size);
		return -1;
	}

	vdb.virt_addr = (unsigned long)vpu_mmap(NULL, vdb.size, PROT_READ | PROT_WRITE, MAP_SHARED, vdi->vpu_fd, vdb.phys_addr);
...
vpu_instance_pool_t *vdi_get_instance_pool(unsigned long coreIdx)
{
	...
	if (!vdi->pvip)
	{
		vdb.size = sizeof(vpu_instance_pool_t) + sizeof(MUTEX_HANDLE)*4;
...
#endif
		if (vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_GET_INSTANCE_POOL, &vdb) < 0)
		{
		...
		vdi->pvip = (vpu_instance_pool_t *)(vdb.virt_addr + (coreIdx*(sizeof(vpu_instance_pool_t) + sizeof(MUTEX_HANDLE)*4)));
		...
		vdi->vpu_omx_mutex = (void *)((unsigned long)vdi->pvip + sizeof(vpu_instance_pool_t) + sizeof(MUTEX_HANDLE));
		vdi->vpu_disp_mutex = (void *)((unsigned long)vdi->pvip + sizeof(vpu_instance_pool_t) + sizeof(MUTEX_HANDLE)*2);
        vdi->vmem_mutex = (void *)((unsigned long)vdi->pvip + sizeof(vpu_instance_pool_t) + sizeof(MUTEX_HANDLE)*3);
...
int vdi_open_instance(unsigned long coreIdx, unsigned long instIdx)
{
	...
	if (vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_OPEN_INSTANCE, &inst_info) < 0)
	{
...
int vdi_close_instance(unsigned long coreIdx, unsigned long instIdx)
{
	...
	inst_info.core_idx = coreIdx;
	inst_info.inst_idx = instIdx;
	if (vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_CLOSE_INSTANCE, &inst_info) < 0)
	{
		VLOG(ERR, "[VDI] fail to deliver open instance num instIdx=%d\n", (int)instIdx);
		return -1;
	}
...
int vdi_hw_reset(unsigned long coreIdx) // DEVICE_ADDR_SW_RESET
{
	...
	return vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_RESET, 0);
		
}

static void resotre_mutex_in_dead(MUTEX_HANDLE *mutex)
{
	int mutex_value;
...
int vdi_lock(unsigned long coreIdx)
{
	...
	resotre_mutex_in_dead((MUTEX_HANDLE *)vdi->vpu_mutex);
...
#ifdef ANDROID
int vdi_omx_lock(unsigned long coreIdx)
{
	vdi_info_t *vdi = &s_vdi_info[coreIdx];

	resotre_mutex_in_dead((MUTEX_HANDLE *)vdi->vpu_omx_mutex);
	pthread_mutex_lock((MUTEX_HANDLE *)vdi->vpu_omx_mutex);
...
void vdi_omx_unlock(unsigned long coreIdx)
{
	...
	pthread_mutex_unlock((MUTEX_HANDLE *)vdi->vpu_omx_mutex);
...
int vdi_disp_lock(unsigned long coreIdx)
{
	...
	resotre_mutex_in_dead((MUTEX_HANDLE *)vdi->vpu_disp_mutex);
...
int vmem_lock(unsigned long coreIdx)
{
	...
	resotre_mutex_in_dead((MUTEX_HANDLE *)vdi->vmem_mutex);
...
extern int vdi_write_register_ffmpeg(unsigned long base_reg_addr, unsigned int addr, unsigned int data);
...
#if (MMP_OS == MMP_OS_WIN32)
    vdi_write_register_ffmpeg(vdi->vdb_register.virt_addr + (coreIdx*VPU_CORE_BASE_OFFSET), addr, data);
#endif
...
extern unsigned int vdi_read_register_ffmpeg(unsigned long base_reg_addr, unsigned int addr);
...
#if (MMP_OS == MMP_OS_WIN32)
    reg_value = vdi_read_register_ffmpeg(vdi->vdb_register.virt_addr + (coreIdx*VPU_CORE_BASE_OFFSET), addr);
#endif

#if (MMP_OS == MMP_OS_WIN32)
    switch(addr) {
        case BIT_CUR_PC:  reg_value=0x1000; break;
    }
#endif
...
int vdi_allocate_dma_memory(unsigned long coreIdx, vpu_buffer_t *vb)
{
	...
	if (vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_ALLOCATE_PHYSICAL_MEMORY, &vdb) < 0)
	{
		VLOG(ERR, "[VDI] fail to vdi_allocate_dma_memory size=%d\n", vb->size);
        vmem_unlock(coreIdx);
        return -1;
...
void vdi_free_dma_memory(unsigned long coreIdx, vpu_buffer_t *vb)
{
	...
	vmem_lock(coreIdx);
	vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_FREE_PHYSICALMEMORY, &vdb);
...
int vdi_set_clock_gate(unsigned long coreIdx, int enable)
{
	...
	ret = vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_SET_CLOCK_GATE, &enable);
...
int vdi_wait_interrupt(unsigned long coreIdx, int timeout, unsigned int addr_bit_int_reason)
{
	...
	vdi_info_t *vdi = &s_vdi_info[coreIdx];
	int ret;
	ret = vpu_ioctl(vdi->vpu_fd, VDI_IOCTL_WAIT_INTERRUPT, timeout);
	if (ret != 0)
		ret = -1;
	return ret;	
#else
