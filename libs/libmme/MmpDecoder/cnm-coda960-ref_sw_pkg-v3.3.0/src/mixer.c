...
typedef enum { YUV444, YUV422, YUV420, NV12, NV21, NV16, NV61, YUV400, YUYV, YVYU, UYVY, VYUY, YYY, RGB_PLANAR, RGB32, RGB24, RGB16, YUV2RGB_COLOR_FORMAT_MAX } yuv2rgb_color_format;
...
yuv2rgb_color_format convert_vpuapi_image_format_to_yuv2rgb_color_format(ImageFormat yuv_format);
...
int sw_mixer_draw(int core_idx, int x, int y, int width, int height, ImageFormat format, unsigned char* pbImage)
{
	...
	color_format = convert_vpuapi_image_format_to_yuv2rgb_color_format(format);
...
yuv2rgb_color_format convert_vpuapi_image_format_to_yuv2rgb_color_format(ImageFormat yuv_format) 
{
	...
	switch(yuv_format)
	{
	case YUV_FORMAT_I420: format = YUV420; break;
	case YUV_FORMAT_NV12: format = NV12; break;
	case YUV_FORMAT_NV21: format = NV21; break;
	case YUV_FORMAT_I422: format = YUV422; break;
	case YUV_FORMAT_NV16: format = NV16; break;	// not support in sw mixer 
	case YUV_FORMAT_NV61: format = NV61; break; // not support in sw mixer  
	case YUV_FORMAT_UYVY: format = UYVY; break; 
	case YUV_FORMAT_YUYV: format = YUYV; break;
	default:
...
#define vpu_clip(var) ((var>=255)?255:(var<=0)?0:var)
	...
	else if( format == YUV422 || format == NV16 || format == NV61 )
	...
	else if( format == NV12 || format == NV21 || format == NV16 || format == NV61) {	
		y = src;
		...
		for( j = 0 ; j < height ; j++ ){
			...
			for( i = 0 ; i < width ; i++ ){
				...
				if (format == YUV422) {
					...
					if (!cbcr_reverse) {
						d = misc[(j)*width+(i>>1<<1)  ] - 128;
						e = misc[(j)*width+(i>>1<<1)+1] - 128;					
					} else {
						e = misc[(j)*width+(i>>1<<1)  ] - 128;
						d = misc[(j)*width+(i>>1<<1)+1] - 128;	
					}
				}
				else { // if (m_color == NV61)
					if (!cbcr_reverse) {
						d = misc[(j)*width+(i>>1<<1)+1] - 128;
						e = misc[(j)*width+(i>>1<<1)  ] - 128;					
					} else {
