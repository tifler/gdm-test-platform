...
OMX_COLOR_FORMATTYPE hal_2_omx_pixel_format(unsigned int hal_format)
{
...
#endif
        default:
            //MMPDEBUGMSG(1, (TEXT("%s:: unsupported csc_hw_type"), __func__));
            break;
        }
    }
    ...
    void *handle)
{
    ...
    pFrameSrc = (AVPicture *)csc_handle->src_buffer.planes[CSC_Y_PLANE];
    
    pFrameDest = &pic;//(AVPicture *)avcodec_alloc_frame();
    memset(pFrameDest, 0x00, sizeof(AVPicture));
    pFrameDest->data[0] = (uint8_t*)csc_handle->dst_buffer.planes[CSC_Y_PLANE];
    pFrameDest->data[1] = (uint8_t*)csc_handle->dst_buffer.planes[CSC_U_PLANE];
    pFrameDest->data[2] = (uint8_t*)csc_handle->dst_buffer.planes[CSC_V_PLANE];
    pFrameDest->linesize[0] = csc_handle->dst_format.width;
    pFrameDest->linesize[1] = csc_handle->dst_format.width>>1;
    pFrameDest->linesize[2] = csc_handle->dst_format.width>>1;
    
    
    //avpicture_fill((AVPicture *)pFrameDest, output, PIX_FMT_YUV420P, csc_handle->dst_format.width, csc_handle->dst_format.height);
    av_picture_copy ((AVPicture *)pFrameDest, (AVPicture*)pFrameSrc, PIX_FMT_YUV420P, csc_handle->dst_format.width, csc_handle->dst_format.height);
