...
CMmpDecoderAudio_Ffmpeg::CMmpDecoderAudio_Ffmpeg(struct MmpDecoderCreateConfig *pCreateConfig) : CMmpDecoderAudio(pCreateConfig), CMmpDecoderFfmpeg(pCreateConfig)
,m_reconfig_count(0)
...
MMP_RESULT CMmpDecoderAudio_Ffmpeg::Open(MMP_U8* pStream, MMP_U32 nStreamSize)
{
...
/*
MMP_RESULT CMmpDecoderAudio_Ffmpeg::AudioConvert_Destroy() {
...
#if 0
MMP_RESULT CMmpDecoderAudio_Ffmpeg::DecodeDSI(MMP_U8* pStream, MMP_U32 nStreamSize) {
...
MMP_RESULT CMmpDecoderAudio_Ffmpeg::DecodeAu(CMmpMediaSample* pMediaSample, CMmpMediaSampleDecodeResult* pDecResult)
{
    MMP_RESULT mmpResult = MMP_SUCCESS;
    ...
    if(frameFinished != 0) {
        ...
        if( (m_pAVCodecContext->sample_rate!=m_wf_in.nSamplesPerSec) 
                ...
                )
            {
                ...
                this->m_uiExpectedDecodedBufferSize = m_pAVFrame_Decoded->nb_samples* m_wf_out.nChannels * m_wf_out.wBitsPerSample/8;
    ...
    if(!m_queue_decoded.IsEmpty()) {
        ...
        m_queue_decoded.Delete(p_decoded_packet);
        buf_remain_size = p_decoded_packet->buf_size - p_decoded_packet->buf_index;
        if((int)pDecResult->uiDecodedBufferMaxSize >= buf_remain_size) {
            buf_copy_size = buf_remain_size;
            ...
            MMPDEBUGMSG(1, (TEXT("[CMmpDecoderAudio_Ffmpeg::DecodeAu] decoded buffer too small (%d %d"), pDecResult->uiDecodedBufferMaxSize, buf_remain_size));
        }
...
void CMmpDecoderAudio_Ffmpeg::AudioConvert_Processing(MMP_U64 uiTimeStamp, AVFrame *pAVFrame_Decoded, AVCodecContext *pAVCodecContext) {
    ...
    if(p_decoded_packet != NULL) {
        ...
        //p_decoded_packet->buf_size = pAVFrame_Decoded->nb_samples * m_wf_out.nChannels * m_wf_out.wBitsPerSample/8; /* ch * sample_byte */
        ...
        if(p_decoded_packet->p_buffer != NULL ) {
            ...
            if(m_pAudioConvert != NULL) {
                ...
                if(iret == 0) {
                    ...
                    if(m_wf_out.nChannels < m_pAVCodecContext->channels) {
