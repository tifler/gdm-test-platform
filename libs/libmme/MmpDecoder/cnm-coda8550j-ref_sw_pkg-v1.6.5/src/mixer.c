...
int  SetMixerDecOutFrame(FRAME_BUF *pFrame, int width, int height)
{
	...
	div = (pFrame->Format == FORMAT_420 || pFrame->Format == FORMAT_422 || pFrame->Format == FORMAT_400) ? 2 : 1;
...
int sw_mixer_open(int instIdx, int width, int height)
{
	...
	{
		JLOG(ERR, "Error during ioctl to get framebuffer parameters!\n");
		return 0;
	}
...
int sw_mixer_draw(int instIdx, int x, int y, int width, int height, int planar_format, int pack_format, int interleave, unsigned char* pbImage)
{		
#ifdef PLATFORM_LINUX
	sw_mixer_context_t *mixer = &s_mixer[instIdx];
	...
	jpu_yuv2rgb(width, height, (yuv2rgb_color_format)convert_jpuapi_format_to_yuv2rgb_color_format(planar_format, pack_format, interleave),
