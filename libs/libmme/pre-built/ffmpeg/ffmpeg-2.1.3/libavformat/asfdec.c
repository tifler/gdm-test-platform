...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
...

    {
        ...
        //int i;
        //int64_t pos, before_pos, index_pos;
        //int64_t index_pts, time_rescale, before_time_scale;
        int pct, ict;
        int64_t itime;
        int i, pktnum, pktct;
        int64_t curpos, nextpos, curpts, pos;
        ...
        if(ict > 0) {
            ...
            curpos = s->data_offset;
            nextpos = s->data_offset + s->packet_size * (int64_t)pktnum;
            curpts = 0;
            
            av_add_index_entry(s->streams[stream_index], curpos, curpts, s->packet_size, 0, AVINDEX_KEYFRAME);
            ...
            for (i = 1; i < ict; i++) {
                ...
                if(pos != nextpos) {
                    ...
                    curpos = nextpos;
                    nextpos = pos;
                    curpts = FFMAX(av_rescale(itime, i, 10000), 0);
