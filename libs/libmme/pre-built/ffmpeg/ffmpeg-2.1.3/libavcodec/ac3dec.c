...
static int s_frame_front = 0;
static int s_frame_rear = 0;

static uint8_t s_frame_buffer[MAX_QUEUE_FRAME][AC3_FRAME_BUFFER_SIZE];
static int  s_frame_size[MAX_QUEUE_FRAME];

static int ac2_buffer_queue_init(void) {
...
static int ac2_buffer_queue_add_tail(uint8_t* buf, int buf_size) {
    ...
    if(s_frame_front == s_frame_rear) // Empty
    {
        return -1;
    }

    sz = s_frame_size[s_frame_front];
    memcpy( &s_frame_buffer[s_frame_front][sz], buf, buf_size);

    s_frame_size[s_frame_front] += buf_size;


    return 0;
}

static int ac2_buffer_queue_add_frame(uint8_t* buf, int buf_size) {
    ...
    i = (s_frame_rear+1)%MAX_QUEUE_FRAME;
    if(s_frame_front == i) // Full
    {
        s_frame_front = (s_frame_front+1)%MAX_QUEUE_FRAME;
    }

    memcpy( &s_frame_buffer[s_frame_rear][0], buf, buf_size);
    s_frame_size[s_frame_rear] = buf_size;

    s_frame_rear = (s_frame_rear+1)%MAX_QUEUE_FRAME;

    return 0;
}


static int ac2_buffer_queue_sync_add(uint8_t* buf_org, int buf_size) {
    
    const uint8_t *buf = buf_org;
    int i,j;
    ...
    for(i = 0; i < buf_size-1; i++) {
        if( (buf[i] == 0x0B) &&  (buf[i+1] == 0x77) ) {
            ...
            ac2_buffer_queue_add_tail(buf_org, i);
            ac2_buffer_queue_add_frame(buf, buf_size);
            break;
        }
    }

    if(i == buf_size-1) {
       // ac2_buffer_queue_add_tail(buf_org, buf_size);
    }

    return 0;
}

static int ac2_buffer_queue_sync_delete(uint8_t** buf_org, int* buf_size) {
    ...
    if(buf_org) *buf_org = s_frame_buffer[s_frame_front];
    if(buf_size) *buf_size = s_frame_size[s_frame_front];
...
static int ac2_buffer_queue_sync_delete_first_item(void) {

    s_frame_front = (s_frame_front+1)%MAX_QUEUE_FRAME;
    return 0;
}

static int ac2_buffer_queue_sync_get_first_item(uint8_t** buf_org, int* buf_size) {
                            ...
                            int *got_frame_ptr, AVPacket *avpkt)
{
    ...
    ac2_buffer_queue_sync_add(avpkt->data, buf_size);
    if(ac2_buffer_queue_sync_get_first_item(&buf, &buf_size) < 0) {
...
#if 0//( (FFMPEG_OS == FFMPEG_OS_WIN32) || (FFMPEG_OS == FFMPEG_OS_ANDROID) )
    for(i = 0; i < buf_size-1; i++) {
    ...
    if (err) {
        ...
        ac2_buffer_queue_sync_delete_first_item();
...
const enum AVSampleFormat c_ac3_dec_sample_fmt[2] = { AV_SAMPLE_FMT_FLTP, AV_SAMPLE_FMT_NONE };
...
AVCodec ff_ac3_decoder = {
    ...
    /*.sample_fmts = */c_ac3_dec_sample_fmt,
...
const enum AVSampleFormat ff_eac3_decoder_sample_fmt[2] = { AV_SAMPLE_FMT_FLTP, AV_SAMPLE_FMT_NONE };
...
AVCodec ff_eac3_decoder = {
    ...
    /*.pix_fmts = */NULL,
    /*.supported_samplerates = */NULL,
    /*.sample_fmts = */ff_eac3_decoder_sample_fmt,
    /*.channel_layouts = */NULL,
#if FF_API_LOWRES
    /*.max_lowres = */0,
#endif
