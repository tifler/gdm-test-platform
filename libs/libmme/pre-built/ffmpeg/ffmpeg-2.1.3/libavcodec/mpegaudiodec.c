...
#define STREAM_BUF_SIZE (LAST_BUF_SIZE+1)
#endif
...
typedef struct MPADecodeContext {
    ...
    AVFrame *frame;
#if ( (FFMPEG_OS == FFMPEG_OS_WIN32) || (FFMPEG_OS == FFMPEG_OS_ANDROID) )
    uint8_t stream_buf_last[1024*64];//LAST_BUF_SIZE];
    int stream_buf_last_size;
    uint8_t stream_buf[1024*32];//[LAST_BUF_SIZE*2];
    int stream_buf_size;
#endif
} MPADecodeContext;
                        ...
                        AVPacket *avpkt)
{
    ...
    int ret;
    int used_byte = 0;
    int stream_buf_size=0, stream_buf_last_size=0;
    int dummy_byte=0;
    int i,j,k;

    if(s->stream_buf_last_size > 0) {
        memcpy(s->stream_buf, s->stream_buf_last, s->stream_buf_last_size);
        stream_buf_size += s->stream_buf_last_size;
        stream_buf_last_size = s->stream_buf_last_size;
        s->stream_buf_last_size = 0;
    }

    memcpy(&s->stream_buf[stream_buf_size], avpkt->data, avpkt->size );
    stream_buf_size += avpkt->size;
    
    k = 0;
    for(i = 0; i < stream_buf_size-4; i++) {
    
        header = AV_RB32(&s->stream_buf[i]);
        if (ff_mpa_check_header(header) == 0) {

#if 1
            j = 4 - ((header >> 17) & 3); //layer
            if(  ((avctx->codec_id == AV_CODEC_ID_MP3) && (j==3))
                || ((avctx->codec_id == AV_CODEC_ID_MP2) && (j==2)) ) {
    ...
    if (s->frame_size <= 0 || s->frame_size > buf_size) {
        ...
        memcpy(s->stream_buf_last, buf, buf_size);
        s->stream_buf_last_size = buf_size;
        *got_frame_ptr = 0;
    ...
    used_byte = dummy_byte+buf_size;
    used_byte -= stream_buf_last_size;
    
    //if(avpack->stream_buf_size > i) {
    //    
    //    s->stream_buf_last_size = stream_buf_size - i;
    //    memcpy(s->stream_buf_last, &s->stream_buf[i], s->stream_buf_last_size);
    //}
                        ...
                        AVPacket *avpkt)
{
    ...
    if(s->stream_buf_last_size == 0) {
        ...
        if (ff_mpa_check_header(header) != 0) {
            sync_error = 1;
            memcpy(s->stream_buf_last, avpkt->data, avpkt->size);
            s->stream_buf_last_size = avpkt->size;
            buf_size = 0;
        }

        used_byte += avpkt->size;
    }

    while(s->stream_buf_last_size > 0) {
        
        k = 0;
        for(i = 0; i < s->stream_buf_last_size-4; i++) {
        
            header = AV_RB32(&s->stream_buf_last[i]);
            if (ff_mpa_check_header(header) == 0) {
                k = 1;
                break;
            }
        ...
        if(k == 1) {
            j = s->stream_buf_last_size-i;
            if(j > 0) {
                memcpy(s->stream_buf, s->stream_buf_last+i, j);

                if(buf_size > 0) {
                    memcpy(&s->stream_buf[j], buf, buf_size);
                    used_byte += buf_size;
                }
                buf = s->stream_buf;
                buf_size += j;
                s->stream_buf_size = buf_size;
            }

            s->stream_buf_last_size = 0;
        }
        else if(sync_error == 0) {

            sync_error = 1;

            av_log(avctx, AV_LOG_DEBUG, "Sync Error\n");
...
#if 0
        /* In case of  incomplete frame,  save current frame to last buffer */
        if(s->stream_buf_last_size == 0) {
            memcpy(s->stream_buf_last, buf, buf_size);
            s->stream_buf_last_size = buf_size;
        }
    ...
    if(s->stream_buf_size > 0) {
        //buf_size= s->frame_size;

        if( s->stream_buf_size > buf_size) {
            
            s->stream_buf_last_size = s->stream_buf_size - buf_size;
            memcpy(s->stream_buf_last, &s->stream_buf[buf_size], s->stream_buf_last_size);
