...
extern int gIspDev;
extern int gInputMode;	// For setting different input mode : OV2715, VIGEN, VSENSOR
extern int gInputModeDevice;
...
uint32_t I2C_Write ( uint16_t usOffset, uint8_t* ptucData, uint8_t ucSize ) {
	...
	if(gInputModeDevice == INPUTMODE_OV2715)
	{
		...
		ioctl(gIspDev, ISP_IOCTL_I2CM_WRITE, gI2C_buf);
	}
	return 0;
}
...
uint32_t I2C_Read ( uint16_t usOffset,uint8_t* ptucData, uint8_t ucSize ) {
	...
	//	memcpy(ptucData, &gI2C_buf[sizeof(ioctl_i2cm_info)], ucSize);
		...
		ioctl(gIspDev, ISP_IOCTL_I2CM_READ, gI2C_buf);
